...
import yt_idv.scene_components
import yt_idv.traitlets_support
...
def getFilePath():
    return pathlib.PurePosixPath(input())
...
async def main():
    ...
    # Enter the file paths from the terminal instead
    ...
    # If the input for field is empty use dens (density) by default
    if field_input == '': field_input = 'dens'
    ...
    color_id = glGenTextures(1)
    glBindTexture(GL_TEXTURE_2D, color_id)
    depth_id =  glGenTextures(1)
