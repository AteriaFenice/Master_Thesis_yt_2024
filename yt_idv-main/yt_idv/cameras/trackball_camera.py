...
class TrackballCamera(BaseCamera):
    ...
    def _update_matrices_view(self, matrix):
        if (not np.array_equal(self.view_matrix, matrix)):
            ...
            print("changed view matrix")
        
    def _update_matrices_value(self, pos, focus, up):
        #self.position = pos
        #self.focus = focus
        self.up = up
        self.view_matrix = get_lookat_matrix(self.position, self.focus, self.up)
    ...
    # Return functions to access relevent information about the camera scene
    def get_cameraposition(self):
        #print("Camera positon: ", self.position)
